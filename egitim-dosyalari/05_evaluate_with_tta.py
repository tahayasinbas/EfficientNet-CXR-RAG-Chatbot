...
def main():
    ...
    # Per-sample TTA tahminleri (hakem #3: Tablo 2 için istatistiksel anlamlılık
    # testi/bootstrap CI gerekiyor — bunun için normal test_predictions.csv ile
    # AYNI FORMATTA, örnek-bazlı TTA tahminleri gerekli; sadece agregat
    # test_metrics_tta.csv yeterli değil).
    tta_predictions_data = {'image_id': image_ids}
    for i, disease in enumerate(config.DISEASES):
        tta_predictions_data[f'{disease}_true'] = y_true[:, i]
        tta_predictions_data[f'{disease}_pred'] = y_pred_tta[:, i]
    df_tta_predictions = pd.DataFrame(tta_predictions_data)
    df_tta_predictions.to_csv(results_dir / 'test_predictions_tta.csv', index=False)

    print(f"\n✅ TTA sonuçları kaydedildi:")
    print(f"  {results_dir / 'test_metrics_tta.csv'}")
    print(f"  {results_dir / 'test_predictions_tta.csv'} (per-sample, {len(df_tta_predictions):,} satır)")
